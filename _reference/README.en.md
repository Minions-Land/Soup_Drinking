...
> **⚠️ Attribution**: All files in this repository EXCEPT `_reference/` come from the briefly open-sourced Claude Code ([github.com/PoorOtterBob/claude-code](https://github.com/PoorOtterBob/claude-code)), copyright Anthropic. That code is not our contribution. **Our entire contribution is in the `_reference/` folder.**
...
```bash
cp -r _reference/skills/agent-design         ~/.claude/skills/
cp -r _reference/skills/find-pattern         ~/.claude/skills/
cp -r _reference/skills/tool-design          ~/.claude/skills/
cp -r _reference/skills/multi-agent-workflow ~/.claude/skills/
```
...
/ (repo root)
│
├── _reference/          ← ✅ OUR CONTRIBUTION (all original content)
...
Contributions welcome. Open a PR to fix errors, add patterns, or improve tutorials. Please maintain the bilingual (EN/ZH) format for all content in `_reference/`.

## License

Content in `_reference/`: MIT License
